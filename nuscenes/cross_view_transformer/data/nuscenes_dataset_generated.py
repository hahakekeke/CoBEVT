...
class NuScenesGeneratedDataset(torch.utils.data.Dataset):
    """
    - object_count is pre-computed from labels at initialization.
    - pre_attention_map and its entropy are dynamically generated from images.
    """
    ...
    def _precompute_object_counts(self):
        """
        [복원] Pre-computes and stores the object_count for each sample from labels.
        """
        nusc_samples_map = {s['token']: i for i, s in enumerate(self.nusc_dataset.samples)}
        ...
        for sample_dict in self.samples:
            ...
            if nusc_idx is None:
                ...
                # NuScenesDataset의 __getitem__은 Sample 객체를 반환한다고 가정
                sample_from_nusc = self.nusc_dataset[nusc_idx]
                object_count = sample_from_nusc.object_count
    ...
    def __getitem__(self, idx):
        # sample_dict에는 라벨 기반의 'object_count'가 이미 포함되어 있습니다.
        sample_dict = self.samples[idx]
        data_obj = Sample(**sample_dict)
        ...
        # 'image' 키가 있는 경우에만 맵과 엔트로피를 동적으로 추가
        if 'image' in data and isinstance(data['image'], torch.Tensor):
            ...
            # AttentionGenerator에서는 맵과 엔트로피만 사용하고, obj_counts는 무시합니다.
            _, pre_att_maps, map_entropies = self.attention_generator.generate(images_for_gen)
            
            # [삭제] 이 라인은 기존의 object_count를 덮어쓰므로 삭제합니다.
