...
class Sample(dict):
    def __init__(
        ...
        bev,
        object_count=0,
        attention_entropy=None, # --- 수정/추가된 부분 ---
        **kwargs
    ):
        super().__init__(**kwargs)
        ...
        self.attention_entropy = attention_entropy # --- 수정/추가된 부분 ---
...
class SaveDataTransform:
    ...
    def get_bev(self, batch: Sample):
        ...
        # Save object_count
        if batch.object_count is not None:
            result['object_count'] = batch.object_count

        # --- 수정/추가된 부분 ---
        # Save attention_entropy
        if batch.attention_entropy is not None:
            # torch.Tensor를 JSON에 저장하기 위해 list로 변환
            result['attention_entropy'] = batch.attention_entropy.tolist()
        # ----------------------
...
class LoadDataTransform(torchvision.transforms.ToTensor):
    ...
    def get_bev(self, sample: Sample):
        ...
        # --- 수정/추가된 부분 ---
        # Load attention_entropy
        if 'attention_entropy' in sample:
            # JSON에서 list로 읽은 값을 tensor로 변환
            result['attention_entropy'] = torch.tensor(sample['attention_entropy'])
        # ----------------------
