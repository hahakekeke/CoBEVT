...
# from .decoder import DecoderBlock # 이 부분은 필요 시 주석 해제

# --- ConvNeXtBlock을 위한 DropPath 추가 ---
# timm 라이브러리가 설치되어 있지 않을 경우를 대비하여 직접 구현
...
class CrossViewSwapAttention(nn.Module):
    ...
    def forward(
        ...
        if object_count is not None:
            ...
            # This is just for debugging print, so no need to change
        else:
...
class PyramidAxialEncoder(nn.Module):
    def __init__(
        ...
        cross_views = list()
        resnet_layers = list()
        convnext_layers = list() # ConvNeXt 레이어 리스트 추가
        ...
        for i, (feat_shape, num_layers) in enumerate(zip(self.backbone.output_shapes, middle)):
            ...
            # 기존 ResNet Bottleneck 레이어 생성
            resnet_layer = nn.Sequential(*[ResNetBottleNeck(dim[i]) for _ in range(num_layers)])
            resnet_layers.append(resnet_layer)

            # ConvNeXt 블록 레이어 생성
            convnext_layer = nn.Sequential(*[ConvNeXtBlock(dim[i]) for _ in range(num_layers)])
            convnext_layers.append(convnext_layer)
        ...
        self.convnext_layers = nn.ModuleList(convnext_layers) # 모듈 리스트로 등록
    ...
    def forward(self, batch):
        ...
        for i, (cross_view, feature) in enumerate(zip(self.cross_views, features)):
            ...
            if object_count is not None:
                ...
                # 객체 수가 30 이상인 배치의 인덱스
                convnext_mask = object_count >= 30
                convnext_indices = convnext_mask.nonzero(as_tuple=True)[0]
                
                # 객체 수가 30 미만인 배치의 인덱스
                resnet_mask = object_count < 30
                resnet_indices = resnet_mask.nonzero(as_tuple=True)[0]

                # ConvNeXt 적용
                if len(convnext_indices) > 0:
                    x_convnext = self.convnext_layers[i](x[convnext_indices])
                    x_out[convnext_indices] = x_convnext
                
                # ResNet 적용
                if len(resnet_indices) > 0:
                    x_resnet = self.resnet_layers[i](x[resnet_indices])
                    x_out[resnet_indices] = x_resnet
                
                x = x_out
            else:
                # object_count가 없으면 기본 ResNet 레이어 사용
...
if __name__ == "__main__":
    # 실행 예제는 제공된 원본 코드와 동일하게 유지합니다.
    # 실제 모델 인스턴스화 및 데이터 전달 시 'object_count'를 배치 딕셔너리에 추가해야 합니다.
    # 예: batch['object_count'] = torch.tensor([10, 35, 5, 40, ...])
    
    # 이 부분은 실제 환경에 맞게 백본 및 설정을 로드해야 정상적으로 동작합니다.
    print("Adaptive Model Code Ready.")
    print("To test, instantiate PyramidAxialEncoder with appropriate configs and pass a batch dictionary containing 'object_count'.")

    # 예시:
    # backbone = ... # 백본 모델 정의
    # params = load_yaml('config/...') # 설정 로드
    # encoder = PyramidAxialEncoder(backbone, **params)
    
    # b = 8 # 배치 사이즈
    # image = torch.rand(b, 6, 3, 224, 224)
    # intrinsics = torch.rand(b, 6, 3, 3)
    # extrinsics = torch.rand(b, 6, 4, 4)
    # object_count = torch.tensor([16, 47, 46, 12, 11, 45, 12, 13])
