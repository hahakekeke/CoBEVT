...
class CrossViewSwapAttention(nn.Module):
    def __init__(
        ...
        if self.bev_embed_flag:
            # img_embed: from 4 channel (world coords) -> dim
    ...
    def forward(
        ...
        if hasattr(self, 'img_embed'):
            d_embed = self.img_embed(d_flat)  # (b n) dim h w
            img_embed = d_embed - c_embed  # (b n) dim h w
            img_embed = img_embed / (img_embed.norm(dim=1, keepdim=True) + 1e-7)
        else:
            img_embed = torch.zeros((b * n, 1, h, w), device=x.device)
        ...
        if self.bev_embed_flag:
            w_embed = self.bev_embed(world[None])  # 1 dim H W
            bev_embed = w_embed - c_embed  # broadcast over (b*n)
            bev_embed = bev_embed / (bev_embed.norm(dim=1, keepdim=True) + 1e-7)
            query_pos = rearrange(bev_embed, '(b n) ... -> b n ...', b=b, n=n)
        else:
            query_pos = None

        feature_flat = rearrange(feature, 'b n ... -> (b n) ...')  # (b n) feat_dim h w

        # ---- SAFELY combine image embedding and feature projection ----
        if self.feature_proj is not None:
            feat_proj = self.feature_proj(feature_flat)  # (b n) dim? h' w'
            # ensure spatial resolution match
            if feat_proj.shape[2:] != img_embed.shape[2:]:
                # interpolate image embedding to feature resolution (non-learned)
                img_embed_resized = F.interpolate(img_embed, size=feat_proj.shape[2:], mode='bilinear', align_corners=False)
            else:
                img_embed_resized = img_embed

            # channel-safe combination: if channels match -> add; otherwise use channel-mean broadcasting
            if img_embed_resized.shape[1] == feat_proj.shape[1]:
                key_flat = img_embed_resized + feat_proj
            else:
                # parameter-free fallback: use channel-mean of img_embed and broadcast to feat_proj channels
                img_stat = img_embed_resized.mean(dim=1, keepdim=True)  # (b n, 1, h, w)
                # expand to feat_proj channels
                key_flat = feat_proj + img_stat.expand(-1, feat_proj.shape[1], -1, -1)
        else:
            # no image feature projection: just use image embedding
            key_flat = img_embed

        val_flat = self.feature_linear(feature_flat)  # (b n) dim h w
        ...
        q_local = rearrange(self.cross_win_attend_1(query_windows, key_windows, val_windows, skip=skip_conn if self.skip else None), 'b x y w1 w2 d -> b (x w1) (y w2) d')
        query = q_local + self.mlp_1(self.prenorm_1(q_local))
        ...
        query = q_global + self.mlp_2(self.prenorm_2(q_global))
