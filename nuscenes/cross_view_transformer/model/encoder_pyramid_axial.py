...
class CrossViewSwapAttention(nn.Module):
    def __init__(
        self, feat_height, feat_width, feat_dim, dim, index, image_height,
        image_width, qkv_bias, q_win_size, feat_win_size, heads, dim_head,
        bev_embedding_flag, rel_pos_emb=False, no_image_features=False,
        skip=True, norm=nn.LayerNorm
    ):
        super().__init__()
...
class PyramidAxialEncoder(nn.Module):
    def __init__(
        ...
        for i, (feat_shape, num_layers) in enumerate(zip(self.backbone.output_shapes, middle)):
            ...
            resnet_layers.append(nn.Sequential(*[ResNetBottleNeck(dim[i]) for _ in range(num_layers)]))
            convnext_layers.append(nn.Sequential(*[ConvNeXtBlock(dim[i]) for _ in range(num_layers)]))
    ...
    def forward(self, batch):
        ...
        for i, (cross_view, feature) in enumerate(zip(self.cross_views, features)):
            ...
            # --- 어댑티브 로직 수정 (DDP 호환) ---
            if object_count is not None:
                # 1. 두 경로의 출력을 모두 계산하여 모든 파라미터가 연산에 참여하도록 함
                out_resnet = self.resnet_layers[i](x)
                out_convnext = self.convnext_layers[i](x)

                # 2. 조건에 따라 마스크 생성
                # (B, 1, 1, 1) 형태로 브로드캐스팅 가능하게 만듦
                mask = (object_count >= 30).view(-1, 1, 1, 1)

                # 3. torch.where를 사용해 마스크에 따라 두 출력 중 하나를 선택
                # 마스크가 True이면 out_convnext, False이면 out_resnet
                x = torch.where(mask, out_convnext, out_resnet)
            else:
...
if __name__ == "__main__":
    print("Adaptive Model Code (DDP Error Fixed) Ready.")
