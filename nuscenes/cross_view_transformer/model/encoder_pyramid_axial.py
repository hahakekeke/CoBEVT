...
class CrossViewSwapAttention(nn.Module):
    ...
    def forward(
        ...
        # --- 수정된 부분 ---
        # 이 모듈에서는 object_count를 직접 사용하지 않고 상위 모듈에서 처리하므로
        # 디버깅 코드는 삭제하여 코드를 정리합니다.
...
# --- 추가된 부분 ---
# object_count 정보를 BEV 특징에 주입하기 위한 간단한 MLP 모듈
class ObjectAdapter(nn.Module):
    ...
    def forward(self, x):
        # object_count는 보통 정수형이므로 float으로 변환
...
class PyramidAxialEncoder(nn.Module):
    def __init__(
        ...
        # --- 수정/추가된 부분 ---
        # 1. ObjectAdapter 모듈 초기화
        self.object_adapter = ObjectAdapter(num_object_classes, dim[0])
        # 2. BEV Self-Attention 모듈 활성화 (주석 해제)
    ...
    def forward(self, batch):
        ...
        # --- 추가된 부분 ---
        # Object_count 정보를 BEV 임베딩에 주입
        if object_count is not None:
            # (b, num_classes) -> (b, dim)
            obj_embedding = self.object_adapter(object_count)
            
            # (b, dim) -> (b, dim, 1, 1) 로 변환하여 BEV 특징에 더함 (브로드캐스팅)
            x = x + obj_embedding[:, :, None, None]
        # --- 추가된 부분 끝 ---
        ...
        for i, (cross_view, feature, layer) in \
            ...
            # object_count를 CrossViewSwapAttention에 전달
