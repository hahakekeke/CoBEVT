...
class PyramidAxialEncoder(nn.Module):
    def __init__(
        ...
        for i, (feat_shape, num_layers) in enumerate(zip(self.backbone.output_shapes, middle)):
            ...
            # [변경] ResNetBottleNeck을 ConvNeXtBlock으로 교체
            layer = nn.Sequential(*[ConvNeXtBlock(dim=dim[i]) for _ in range(num_layers)])
            layers.append(layer)
...
if __name__ == "__main__":
    ...
    ##### EncoderSwap
    # 이 부분은 설정 파일과 백본 모델의 실제 인스턴스가 필요하므로
    # 그대로 실행하면 오류가 발생할 수 있습니다.
