...
class CrossWinAttention(nn.Module):
    ...
    def forward(self, q, k, v, skip=None, temperature: Optional[torch.Tensor] = None):
        ...
        # ✅ temperature 적용
        scale = self.scale if temperature is None else (self.scale / temperature)
        dot = dot * scale
...
class CrossViewSwapAttention(nn.Module):
    def __init__(self, feat_height: int, feat_width: int, feat_dim: int, dim: int,
                 index: int, image_height: int, image_width: int,
                 qkv_bias: bool, q_win_size: list, feat_win_size: list,
                 heads: list, dim_head: list, bev_embedding_flag: list,
    ...
    def forward(self, index: int, x: torch.FloatTensor, bev: BEVEmbedding,
                feature: torch.FloatTensor, I_inv: torch.FloatTensor,
                E_inv: torch.FloatTensor, object_count: Optional[torch.Tensor] = None):
        # ✅ object_count 기반 temperature 계산
        if object_count is not None:
            avg_obj = object_count.float().mean().item()
            temperature = max(0.5, min(2.0, 20.0 / (avg_obj + 1e-6)))  # 객체 많으면 0.5쪽, 적으면 2.0쪽
        else:
            temperature = 1.0
        ...
        c = E_inv[..., -1:]; c_flat = rearrange(c, 'b n ... -> (b n) ...')[..., None]
